...
router.post("/list", validateApiKey, async (req, res) => {
  ...
  response = await callAPI(URL_COLLME, "transgl/list", req.body, {
...
router.post("/master", validateApiKey, async (req, res) => {
  ...
  response = await callAPI(URL_COLLME, "transgl/master", req.body, {
    "x-api-key": "X0pQMBDSo2DAarIgOQrbm/N/UySKrbPeo3QbnZkKD7M=",
  });
...
router.post("/insert", validateApiKey, async (req, res) => {
  ...
  response = await callAPI(URL_COLLME, "transgl/insert", req.body, {
    "x-api-key": "X0pQMBDSo2DAarIgOQrbm/N/UySKrbPeo3QbnZkKD7M=",
  });
